  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const totalSpent = contractMoneyResult.data?.reduce((sum, item) => sum + item.total_spent, 0) || 0
  ...
  useEffect(() => {
    if (!loading) {
      ...
      if (contractMoneyChartRef.current && contractMoneyData.length > 0) {
        ...
        const data = contractMoneyData.map((item) => item.total_spent)
      ...
      <div className="stats-grid">
        <div className="stat-card">
          ...
          <div className="stat-content">
            ...
            <p className="stat-change positive">Lifetime spending</p>
          </div>
        </div>
